...
module YAVS
  module Sync
    def self.sync_ftp host, user, passwd, path
      log "Connecting to ftp://#{user}:#{passwd}@#{host}"
      ftp = BetterFTP.new host, user, passwd

      log "Creating archive"
      Archive::Zip.archive("#{Dir.home}/.tempyavsextract.zip", '.')

      log "Uploading as #{path}"
      ftp.putbinaryfile(File.new("#{Dir.home}/.tempyavsextract.zip"), path)

      File.delete "#{Dir.home}/.tempyavsextract.zip"
